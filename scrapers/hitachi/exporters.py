"""
Export functions for Hitachi vulnerability data.
"""
import json
...
from models import HitachiVulnerability


def export_to_json(vulnerabilities: list[HitachiVulnerability], output_path: str) -> None:
    ...
    data = {
        "metadata": {
            ...
            "url": "https://www.hitachi.com/products/it/software/security/index.html"
        },
...
def export_to_excel(vulnerabilities: list[HitachiVulnerability], output_path: str) -> None:
    """Export vulnerabilities to Excel file with multiple sheets."""
    wb = Workbook()
    
    # Styles
    header_font = Font(bold=True, color="FFFFFF")
    header_fill = PatternFill(start_color="e60012", end_color="e60012", fill_type="solid")  # Hitachi red
    ...
    overview_headers = [
        "Advisory ID", "Title", "Severity", "Max CVSS",
        "CVE IDs", "Publication Date", "Last Update", "Description"
    ]
    ...
    for row, vuln in enumerate(vulnerabilities, 2):
        ws_overview.cell(row=row, column=1, value=vuln.advisory_id).border = thin_border
        ws_overview.cell(row=row, column=2, value=vuln.title).border = thin_border
        
        severity_cell = ws_overview.cell(row=row, column=3, value=vuln.severity)
        ...
        ws_overview.cell(row=row, column=4, value=vuln.max_cvss_score).border = thin_border
        ws_overview.cell(row=row, column=5, value=", ".join(vuln.cve_ids)).border = thin_border
        ws_overview.cell(row=row, column=6, value=vuln.publication_date).border = thin_border
        ws_overview.cell(row=row, column=7, value=vuln.last_update).border = thin_border
        desc = vuln.description[:500] + "..." if len(vuln.description) > 500 else vuln.description
        ws_overview.cell(row=row, column=8, value=desc).border = thin_border
    
    # Adjust column widths for overview
    overview_widths = [22, 70, 12, 10, 35, 15, 15, 80]
    for col, width in enumerate(overview_widths, 1):
        ws_overview.column_dimensions[get_column_letter(col)].width = width
    
    # Sheet 2: CVE Details
    ws_cve = wb.create_sheet("CVE Details")
    
    cve_headers = ["Advisory ID", "CVE ID", "CVSS Score", "Severity", "Title"]
    ...
    for vuln in vulnerabilities:
        for cve_id in vuln.cve_ids:
            ws_cve.cell(row=cve_row, column=1, value=vuln.advisory_id).border = thin_border
            ws_cve.cell(row=cve_row, column=2, value=cve_id).border = thin_border
            cvss = vuln.cvss_scores.get(cve_id)
            ws_cve.cell(row=cve_row, column=3, value=cvss).border = thin_border
            
            # Determine severity from this specific CVE's score
            if cvss:
                if cvss >= 9.0:
                    sev = "Critical"
                elif cvss >= 7.0:
            ...
            severity_cell = ws_cve.cell(row=cve_row, column=4, value=sev)
            severity_cell.border = thin_border
            severity_cell.fill = severity_fills.get(sev, severity_fills["Unknown"])
            severity_cell.font = Font(color="FFFFFF", bold=True)
            
            ws_cve.cell(row=cve_row, column=5, value=vuln.title).border = thin_border
            cve_row += 1
    
    cve_widths = [22, 20, 12, 12, 70]
    for col, width in enumerate(cve_widths, 1):
        ws_cve.column_dimensions[get_column_letter(col)].width = width
    
    # Sheet 3: Affected Products
    ws_products = wb.create_sheet("Affected Products")
    
    product_headers = ["Advisory ID", "Product", "Affected Versions", "Fixed Version", "CVE ID"]
    ...
    for vuln in vulnerabilities:
        for product in vuln.affected_products:
            if isinstance(product, dict):
                ws_products.cell(row=product_row, column=1, value=vuln.advisory_id).border = thin_border
                ...
                ws_products.cell(row=product_row, column=4, value=product.get('fixed_version', '')).border = thin_border
                ...
                ws_products.cell(row=product_row, column=4, value=product.fixed_version).border = thin_border
                ws_products.cell(row=product_row, column=5, value=product.cve_id or '').border = thin_border
            product_row += 1
    
    product_widths = [22, 50, 30, 30, 20]
    ...
    for row, vuln in enumerate(vulnerabilities, 2):
        ws_links.cell(row=row, column=1, value=vuln.advisory_id).border = thin_border
